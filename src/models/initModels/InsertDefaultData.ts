import { Op } from "sequelize";
import { Sequelize } from "sequelize";
import bcrypt from "bcrypt";
import { NamesProfiles } from "../../routes/authorizations/LevelAccesModels";
export class InsertDataInitDb {
  //TODO
  public static async initialiseurDefaultData(
    sequelize: Sequelize
  ): Promise<void> {
    await Promise.all([
      //  InsertDataInitDb.insDefaultPersonnel(sequelize),
      InsertDataInitDb.insDefaultPoste(sequelize),
      InsertDataInitDb.insDefaultTypeDeSalle(sequelize),
      InsertDataInitDb.insDefaultTypeDeRV(sequelize),
    ]);
  }
  /*
  //TODO
  private static async insDefaultPersonnel(
    sequelize: Sequelize
  ): Promise<void> {
    const modelDefaultAdmin = sequelize.models.Personnel;
    const dataDefautlPersonnel = await modelDefaultAdmin.findOne({
      where: {
        [Op.or]: [
          { email: "ecmbayeclement@gmail.com" },
          { email: "ndieyediop@gmail.com" },
          { email: "moussandiaye@gmail.com" },
          { email: "malickyade@gmail.com" },
        ],
      },
    });
    if (!dataDefautlPersonnel) {
      try {
        //HASH
        const salt = await bcrypt.genSalt(10);
        const pwdHashAdmin = await bcrypt.hash("Firstadmin01", salt);
        const pwdHashMed = await bcrypt.hash("Firstmed01", salt);
        const pwdHashInf = await bcrypt.hash("Firstinf01", salt);
        const pwdHashSecr = await bcrypt.hash("Firstsec01", salt);
        await Promise.all([
          //
          modelDefaultAdmin.create({
            nom: "DIOP",
            prenom: "Ndieye",
            email: "ndieyediop@gmail.com",
            telephone: "777777777",
            adresse: "Dakar",
            age: 0,
            sexe: "F",
            role: NamesProfiles.medecin,
            mdp: pwdHashMed,
          }),
          //
          modelDefaultAdmin.create({
            nom: "NDIAYE",
            prenom: "Moussa",
            email: "moussandiaye@gmail.com",
            telephone: "777777777",
            adresse: "Thies",
            age: 0,
            sexe: "M",
            role: NamesProfiles.infirmier,
            mdp: pwdHashInf,
          }),
          //
          modelDefaultAdmin.create({
            nom: "YADE",
            prenom: "Malick",
            email: "malickyade@gmail.com",
            telephone: "777777777",
            adresse: "Thies",
            age: 0,
            sexe: "M",
            role: NamesProfiles.secretaire,
            mdp: pwdHashSecr,
          }),
          //
          modelDefaultAdmin.create({
            nom: "MBAYE",
            prenom: "Etienne Clement",
            email: "ecmbayeclement@gmail.com",
            telephone: "777777777",
            adresse: "Dakar",
            age: 0,
            sexe: "M",
            role: NamesProfiles.admin,
            mdp: pwdHashAdmin,
          }),
        ]);
      } catch (error) {
        throw new Error(
          "Une erreut c'est produite avec les personnels par défaut : " + error
        );
      }
    }
  }
  */
  //TODO
  private static async insDefaultPoste(sequelize: Sequelize): Promise<void> {
    const modelDefaultPoste = sequelize.models.Poste;
    const dataDefautlPoste = await modelDefaultPoste.findAll({
      limit: 1,
    });
    if (dataDefautlPoste.length < 1) {
      try {
        await Promise.all([
          modelDefaultPoste.create({
            poste: NamesProfiles.admin,
          }),
          modelDefaultPoste.create({
            poste: NamesProfiles.medecin,
          }),
          modelDefaultPoste.create({
            poste: NamesProfiles.infirmier,
          }),
          modelDefaultPoste.create({
            poste: NamesProfiles.secretaire,
          }),
        ]);
      } catch (error) {
        throw new Error(
          "Une erreut c'est produite avec les poste par defaut : " + error
        );
      }
    }
  }
  //TODO
  private static async insDefaultTypeDeSalle(
    sequelize: Sequelize
  ): Promise<void> {
    const modelDefaultTypeDeSalle = sequelize.models.TypeDeSalle;
    const dataDefautlTypeDeSalle = await modelDefaultTypeDeSalle.findAll({
      limit: 1,
    });
    if (dataDefautlTypeDeSalle.length < 1) {
      try {
        await Promise.all([
          modelDefaultTypeDeSalle.create({
            type: "Cabinet",
          }),
          modelDefaultTypeDeSalle.create({
            type: "Commune",
          }),
        ]);
      } catch (error) {
        throw new Error(
          "Une erreut c'est produite avec les poste par defaut : " + error
        );
      }
    }
  }
  //TODO
  private static async insDefaultTypeDeRV(sequelize: Sequelize): Promise<void> {
    const modelDefaultTypeRD = sequelize.models.TypeRendezVous;
    const dataDefautlTypeDeSalle = await modelDefaultTypeRD.findAll({
      limit: 1,
    });
    if (dataDefautlTypeDeSalle.length < 1) {
      try {
        await Promise.all([
          modelDefaultTypeRD.create({
            type: "Premiere consultation",
            prix: 2000,
          }),
          modelDefaultTypeRD.create({
            type: "Consultation de suivi",
            prix: 1000,
          }),
          modelDefaultTypeRD.create({
            type: "Suivi gynécologique",
            prix: 4000,
          }),
          modelDefaultTypeRD.create({
            type: "Suivi de nourrisson et de l'enfant",
            prix: 2500,
          }),
        ]);
      } catch (error) {
        throw new Error(
          "Une erreut c'est produite avec les types de rendez-vous par defaut : " +
            error
        );
      }
    }
  }
}
